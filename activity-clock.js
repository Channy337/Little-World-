(function(){
  ...
  function enhance(data){
    ...
    var now=nowMs(),dt=clamp((now-lastUpdate)/1000,0,MAX_FRAME_SECONDS);lastUpdate=now;var t=now/1000,alive=new Set();
    data.state.agents.forEach(function(a){
      alive.add(a.id);var p=visualAgents.get(a.id);if(!p)p={x:a.x,y:a.y};
      if(p.canonicalX!==undefined&&distance(p.canonicalX,p.canonicalY,a.x,a.y)>120)p={x:a.x,y:a.y};p.canonicalX=a.x;p.canonicalY=a.y;
      ...
      if(target&&!a.pending?.home&&a.state!=='resting'&&a.state!=='socializing'&&a.action?.targetType!=='self'){
        ...
        if(p.key!==key){p.key=key;p.phase='outbound';p.timer=0;var home=(data.state.buildings||[]).find(function(b){return b.id===a.home;});var base=home||data.state.market||data.state.well||{x:a.x,y:a.y};p.base={x:base.x,y:base.y};if(distance(p.base.x,p.base.y,target.x,target.y)<16)p.base={x:clamp(target.x-40,8,472),y:clamp(target.y+24,8,296)};}
        if(p.phase==='outbound'){displayState='moving';if(moveToward(p,target.x,target.y,VISUAL_MOVE_SPEED*dt)){p.phase='work';p.timer=10;displayState='working';}}
        else if(p.phase==='work'){displayState='working';p.timer-=dt;if(p.timer<=0){p.phase='return';displayState='moving';}}
        else if(p.phase==='return'){displayState='moving';if(moveToward(p,p.base.x,p.base.y,VISUAL_MOVE_SPEED*dt)){p.phase='pause';p.timer=5;displayState='idle';}}
      ...
      }else{p.key=null;p.phase=null;if(a.state==='moving'&&finite(a.tx)&&finite(a.ty)){if(moveToward(p,a.tx,a.ty,VISUAL_MOVE_SPEED*dt))displayState=visualArrivalState(a);}else moveToward(p,a.x,a.y,VISUAL_MOVE_SPEED*dt);}
      var off=p.key?{x:0,y:0}:ambientOffset({id:a.id,state:displayState},t);a.x=clamp(p.x+off.x,0,480);a.y=clamp(p.y+off.y,0,304);a.state=displayState;visualAgents.set(a.id,p);
    });
