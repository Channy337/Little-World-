...
// Token instrumentation for cost measurement. Process-local; resets on cold start.
// Structured logs appear in Vercel function logs. getUsageStats() exposes totals.
const usageStats = {
  calls: 0,
  inputTokens: 0,
  outputTokens: 0,
...
function recordUsage(usage) {
  ...
  usageStats.calls += 1;
  usageStats.inputTokens += input;
  usageStats.outputTokens += output;
  // Structured log for easy grep in Vercel logs during the 1-hour measurement.
  console.log(JSON.stringify({
    type: 'civoria_ai_usage',
    model: MODEL,
    input_tokens: input,
    output_tokens: output,
    total_calls: usageStats.calls,
    total_input_tokens: usageStats.inputTokens,
    total_output_tokens: usageStats.outputTokens,
    elapsed_ms: Date.now() - usageStats.startedAt
  }));
}
...
function getUsageStats() {
  return {
    ...
    elapsedMs: Date.now() - usageStats.startedAt,
    model: MODEL
  };
}
...
const SAFETY_NOTE = "Keep it cozy and low-stakes: no graphic violence, no adult content. " +
  ...
  "Do not claim a hypothesis is true before Civoria tests it. Never reveal chemical names, formulas, atoms, molecules, scientific species names, or invisible properties unless this person has an observation from a capable instrument. Respond with ONLY the JSON object, nothing else.";
...
function mindLine(mind) {
  ...
  const observations=(mind.observations||[]).map(x=>x.text).join(' | ') || 'only ordinary senses so far';
  const instructions=(mind.instructions||[]).map(x=>x.id+' ('+x.status+')').join(' | ') || 'none received';
  ...
  return [
    ...
    'Personally observed evidence: '+observations+'.',
    'Methods other people described, not yet proven personally: '+instructions+'.',
...
function buildPrompt(item) {
  if (item.type === 'priority') {
    const materialNames={fiber:'plant strands',clay:'sticky earth',ore:'heavy reddish stone',bone:'hard animal remains',hide:'animal skin',plantbit:'small hard pieces saved from fruit'};
    const extraMaterials=Object.entries(item.materials||{}).filter(([,n])=>n>0).map(([k,n])=>n+' '+(materialNames[k]||k));
    const carried=[item.food>0?item.food+' gathered food':'',item.wood>0?item.wood+' wood':'',item.stone>0?item.stone+' stone':'',...extraMaterials,item.coins>0?item.coins+' small marked objects':''].filter(Boolean);
    return [
      ...
      'Right now: hunger ' + item.hunger + '/100 (higher = hungrier), hydration reserve about ' + item.hydration + '/100, energy ' + item.energy + '/100, alertness '+item.alertness+'/100, social comfort ' + item.social + '/100. ' +
      ...
      'Other things directly visible now: '+(item.surroundings||'nothing distinct')+'.',
      item.appearance ? 'Visible appearance: '+item.appearance.skin+' skin, '+item.appearance.hair+' '+item.appearance.hairShade+' hair, and '+item.appearance.eyes+' eyes. These are observations, not racial, cultural, or behavioral categories.' : '',
      'Their communication is still emerging through gesture, voice, and '+((item.communication&&item.communication.sharedSigns)||0)+' shared signs. Render their inner experience in English only as a visitor translation.',
      'Weather they can directly sense: '+(item.environment||'ordinary outdoor conditions')+'.',
      item.symptoms&&item.symptoms.length ? ('Body sensations they personally feel: '+item.symptoms.join(' | ')+'.') : '',
      mindLine(item.mind),
      '',
      ...
      'If they have a concrete idea worth testing, choose a broad physical operation and a simple visible form. Materials must be things they currently carry; body means only non-invasive self-observation. Placing or burying something is allowed, but that does not imply they know what will happen later. Otherwise set experiment to null.',
      'A hopedResult is only what they expect to notice, not a fact. Never jump from an idea to a proven technology, recipe, farming concept, or scientific explanation.',
      'Choose what matters most right now. ' + SAFETY_NOTE,
      'Respond with ONLY this JSON: {"focus":"hunger"|"energy"|"social"|"work","thought":"first-person, under 14 words","goal":"personal goal under 12 words","reflection":"what this experience means to them, under 12 words","lesson":"only a lesson supported by their memories/current evidence, under 10 words or empty","belief":"subjective belief grounded in their experience, under 10 words or empty","experiment":null|{"hypothesis":"idea under 18 words","operation":"observe|combine|shape|stack|soak|dry|heat|compare|bind|balance|spin|channel|place|bury","materials":["wood"|"stone"|"fiber"|"clay"|"ore"|"bone"|"hide"|"food"|"plantbit"|"body"],"form":"rough|thin|flat|long|round|hollow|pointed|frame|sheet|cord","hopedResult":"expected observation under 14 words"}}'
    ].filter(Boolean).join('\n');
  }
  if (item.type === 'chat') {
    return [
...
module.exports = { resolveBatch, MODEL, MAX_PER_INVOCATION, buildPrompt, getUsageStats };
