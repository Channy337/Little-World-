...
function sleepQuality(agent,{weather={},sheltered=false}={}){
  ...
  const air=Number.isFinite(weather.feelsLikeC)?weather.feelsLikeC:(Number.isFinite(weather.temperatureC)?weather.temperatureC:18);
  if(!sheltered){
    quality-=Math.min(.38,Math.abs(air-18)*.018);
    if((weather.precipitationMm||0)>.2)quality-=.22;
    if((weather.windKph||0)>30)quality-=.1;
    if((weather.snowDepthCm||0)>=8)quality-=.08;
    if(weather.hazard==='blizzard')quality-=.12;
  }
  if((agent.thirst||0)>55)quality-=.12;
  if((agent.hunger||0)>80)quality-=.12;
  return clamp(quality,.15,1);
}
...
function symptomTexts(body){
  ...
  if(body.coreTemperature<=34)out.push('The cold is making me clumsy, sleepy, and confused.');
...
function advanceBody(agent,{weather={},active=false,sheltered=false,days=0,waterLossPerDay=24}={}){
  const b=ensureBody(agent),d=Math.max(0,days);
  const air=Number.isFinite(weather.temperatureC)?weather.temperatureC:18;
  const felt=Number.isFinite(weather.feelsLikeC)?weather.feelsLikeC:air;
  const exposedCold=Math.max(0,12-felt)*(sheltered?.16:1);
  const wetCold=!sheltered&&(weather.precipitationMm||0)>0&&air<=6?2.5:0;
  const coldDemand=(exposedCold+wetCold)*.018;
  const demand=(active?1.18:1)+coldDemand;
  ...
  const exposure=sheltered?.16:1;
  const wetFactor=!sheltered&&(weather.precipitationMm||0)>0&&air<=6?1.35:1;
  const snowFactor=!sheltered&&(weather.snowDepthCm||0)>=8?1.15:1;
  const target=37+Math.max(0,air-27)*.12*exposure-Math.max(0,8-felt)*.18*exposure*wetFactor*snowFactor;
  b.coreTemperature+=clamp(target-b.coreTemperature,-1.45*d,1.2*d);
  ...
  if(b.coreTemperature<35){const severity=35-b.coreTemperature;agent.health=clamp((Number.isFinite(agent.health)?agent.health:100)-severity*9*d,0,100);}
