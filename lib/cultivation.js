...
function collectSeed(state,agent,bush,day){
  ...
  seeds.push(seed);if(seeds.length>24)seeds.splice(0,seeds.length-24);ensureAgentSeeds(agent);
noteSeedSense(agent,'Inside this fruit are '+p.seedAppearance+'. I still carry some of them.',day);
return seed;
}
function noteSeedSense(agent,text,day){
if(!agent||!agent.mind)return;
if(!Array.isArray(agent.mind.observations))agent.mind.observations=[];
if(agent.mind.observations.some(x=>x.text===text))return;
agent.mind.observations.push({text,day:Number.isSafeInteger(day)?day:0,source:'senses'});
if(agent.mind.observations.length>24)agent.mind.observations.splice(0,agent.mind.observations.length-24);
}
