...
function profileForBush(bush){
  ...
  if(!PLANT_SPECIES[bush.plantSpecies])bush.plantSpecies=SPECIES_KEYS[Math.abs(Number(bush.id)||0)%SPECIES_KEYS.length];
...
function ensureAgentSeeds(agent){if(!Array.isArray(agent.seeds))agent.seeds=[];if(!agent.cultivationEvidence||typeof agent.cultivationEvidence!=='object')agent.cultivationEvidence={};return agent.seeds;}
...
function collectSeed(state,agent,bush,day){
  ensureCultivation(state);const seeds=ensureAgentSeeds(agent),p=profileForBush(bush),g=bush.plantGenome||genomeFor(bush.id,p);
  const seed={id:state.nextId++,sourceId:bush.id,species:bush.plantSpecies,appearance:p.seedAppearance,collectedDay:day,viability:1,genome:{...g}};
  seeds.push(seed);if(seeds.length>24)seeds.splice(0,seeds.length-24);return seed;
}
...
function plantSeed(state,agent,{x,y,depthCm=2,watered=false}={}){
  ...
  const at=agent.seeds.findIndex(x=>x.id===seed.id);agent.seeds.splice(at,1);
...
function advanceCultivation(state,days){
  ensureCultivation(state);const d=Math.max(0,Number(days)||0),w=state.weather||{},soilMoisture=clamp(Number.isFinite(w.soilMoisture)?w.soilMoisture:.5,0,1),temp=Number.isFinite(w.temperatureC)?w.temperatureC:18;
  for(const q of state.plantings){
    ...
    if(q.stage==='dormant'){
      ...
      else if(q.ageDays>g.dormancyDays+35&&(temperatureOk===false||moistureOk===false))q.viability=clamp(q.viability-d*.012,0,1);
...
module.exports={PLANT_SPECIES,SPECIES_KEYS,ensureCultivation,ensureAgentSeeds,profileForBush,collectSeed,seedCount,firstViableSeed,plantSeed,waterPlanting,advanceCultivation,sensoryPlant,hiddenIdentity};
