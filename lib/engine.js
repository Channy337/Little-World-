...
const {createBody,ensureBody,drink:drinkBody,eat:eatBody,advanceBody}=require('./biology');
...
if(shelter.assign(S,a,log))return;
...
if(a.thirst>=50){goTo(a,S.pond.x,S.pond.y,{targetType:'water',sub:'drink'});return;}
if(a.hunger>=92){
...
} // Legacy in-flight house actions return to planning; no instant house bypass.
break;
case 'self':
if(act.sub==='eat' && a.inv.food>0){ a.inv.food-=1; eatBody(a,1); }
break;
}
...
if(Number.isFinite(a.health)&&a.health<=0){a.dead=true;a.deathReason='burn injuries';return;}
...
a.aiCooldown -= dt;
ensureBody(a,random);
var bodyDeath=advanceBody(a,{weather:S.weather,active:busy,sheltered:!!a.home&&(a.state==='resting'||a.state==='idle'),days:dt/CFG.dayLength,waterLossPerDay:waterLossPerDay(S.weather,busy)});
...
if(bodyDeath){a.dead=true;a.deathReason=bodyDeath;return;}
...
if(a.shelterJob&&shelter.step(S,a,dt,log))return;
...
var i;
if(S.constructionSites)shelter.repair(S,log);
...
if(!Array.isArray(S.fires))S.fires=[];if(!Array.isArray(S.records))S.records=[];if(!S.discoveryGraph)S.discoveryGraph={nodes:[],edges:[]};if(!Number.isFinite(S.ambientTemperature))S.ambientTemperature=18;if(!Array.isArray(S.weatherHistory))S.weatherHistory=[];ensureWeather(S,random);if(S.pond)S.pond.level=S.weather.pondLevel;
for(var savedAgent of S.agents){if(!savedAgent.inv)savedAgent.inv={wood:0,stone:0,food:0};if(!Number.isFinite(savedAgent.inv.timber))savedAgent.inv.timber=0;if(!Number.isFinite(savedAgent.thirst))savedAgent.thirst=20;if(!Number.isFinite(savedAgent.health))savedAgent.health=100;if(!Number.isFinite(savedAgent.pain))savedAgent.pain=0;if(!Number.isFinite(savedAgent.bodyTemperature))savedAgent.bodyTemperature=37;if(typeof savedAgent.literacy!=='boolean')savedAgent.literacy=false;if(!Array.isArray(savedAgent.tools))savedAgent.tools=[];ensureBody(savedAgent,random);savedAgent.thirst=100-savedAgent.body.waterReserve;}
} else initWorld();
