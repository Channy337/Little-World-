...
materials:{fiber:a.inv.fiber||0,clay:a.inv.clay||0,ore:a.inv.ore||0,bone:a.inv.bone||0,hide:a.inv.hide||0,plantbit:a.inv.plantbit||0},
...
S.trees = []; S.rocks = []; S.bushes = []; S.farms = []; S.buildings = []; S.market = null; S.agents = [];S.deposits=[];S.constructionSites=[];
...
if(b && b.food>=1){ b.food-=1;forageFood(a,b,S.day,S); }
...
// Legacy in-flight house actions fall back to the persistent shelter planner.
if(!a.home)shelter.assign(S,a,log);
}
break;
case 'self':
