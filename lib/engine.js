...
tools:[],
starveTimer:0,dehydrationTimer:0,dead:false
};
}
...
S.log = []; S.chronicle = []; S.nextId = 1;S.fires=[];S.records=[];S.discoveryGraph={nodes:[],edges:[]};S.ambientTemperature=18;
...
if(a.thirst>=88){goTo(a,S.pond.x,S.pond.y,{targetType:'water',sub:'drink'});return;}
if(a.hunger>=92){
...
case 'water':
if(act.sub==='drink'){
a.thirst=Math.max(0,a.thirst-72);
...
a.thirst=clamp((Number.isFinite(a.thirst)?a.thirst:20)+CFG.thirstRate*dt,0,100);
...
}else a.bodyTemperature+=(37-(a.bodyTemperature||37))*.08*dt;

if(a.hunger>=99) a.starveTimer=(a.starveTimer||0)+dt; else a.starveTimer=0;
if(a.thirst>=99)a.dehydrationTimer=(a.dehydrationTimer||0)+dt;else a.dehydrationTimer=0;
if(a.starveTimer>10){ a.dead=true; a.deathReason='hunger'; return; }
if(a.dehydrationTimer>12){a.dead=true;a.deathReason='dehydration';return;}
...
if(S.day!==previousDay&&random()<.08&&S.trees.length){
...
var i;
for(i=(S.fires||[]).length-1;i>=0;i--){var flame=S.fires[i];flame.fuel-=dt*.055;flame.intensity=clamp(flame.fuel/2,0,1.4);if(flame.fuel<=0)S.fires.splice(i,1);}
