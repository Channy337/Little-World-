...
const MATERIALS=new Set(['wood','stone','food','body']);
  ...
  for(const a of state.agents||[]){
    ...
    if(evidence>=2){
      ...
      if(!m.capabilities.some(x=>x.id===key)){
        m.capabilities.push({id:key,kind:rule.capabilityKind||'recipe',evidence,discoveredDay:state.day});
        ...
        recordDiscovery(state,{id:key,label:rule.label||key,kind:rule.capabilityKind==='observation'?'body-observation':'process',agentId:a.id,day:state.day,evidence:m.experiments.filter(x=>signature(x.operation,x.materials)===key).map(x=>x.hypothesis)});
      }
    }
  }
  return state;
