...
const {verifyGitHubOidc}=require('./github-oidc');

async function scheduled(req,env,oidcVerifier) {
  ...
  if(expected && Buffer.byteLength(actual)===Buffer.byteLength(expected) &&
    timingSafeEqual(Buffer.from(actual),Buffer.from(expected))) return true;
  const token=actual.slice(7);
  if(!token || !token.includes('.')) return false;
  return !!(await oidcVerifier(token));
}
function makeHandler(kind,{service=createWorldService(),env=process.env,logger=console.error,oidcVerifier=verifyGitHubOidc}={}) {
  return async function handler(req,res) {
    ...
    if(kind==='heartbeat') {
      if(!(await scheduled(req,env,oidcVerifier))) return res.status(401).json({error:'unauthorized',requestId});
    ...
    if(kind==='tick' && req.method==='GET' && !(await scheduled(req,env,oidcVerifier))) return res.status(401).json({error:'unauthorized',requestId});
...
module.exports={makeHandler,scheduled};
