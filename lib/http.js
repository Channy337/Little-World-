...
function makeHandler(kind,{service=createWorldService(),env=process.env,logger=console.error}={}) {
  return async function handler(req,res) {
    ...
    if(kind==='state' && req.method!=='GET' || kind==='tick' && !['GET','POST'].includes(req.method)) {
    ...
    try {return res.status(200).json(await service[kind]());}
      ...
      const known=['storage_not_configured','storage_unavailable','storage_command_failed','world_invalid','world_missing_restore_required','preview_branch_missing','environment_missing'];
