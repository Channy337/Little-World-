...
const {clarity}=require('./communication');
...
function transferKnowledge(state,teacher,learner){
  ...
  const trust=rel?rel.trust:50,score=(teacher.id*31+learner.id*17+state.day*13)%100,understanding=clarity(teacher,learner);
  if(score>Math.min(88,18+trust*.4+understanding*30))return null;
