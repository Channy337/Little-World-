...
function landScore(seed,x,y){
  ...
  if(ellipse(x,y,36,9,6,4.4)<1)score-=1.1;
  if(ellipse(x,y,39,27,8,5)<1)score-=.72;
...
function coastDistance(seed,x,y){
  let best=8;
  for(let r=1;r<=7;r++){
    for(let dx=-r;dx<=r;dx++)for(const dy of [-r,r])if(x+dx>=0&&x+dx<WIDTH&&y+dy>=0&&y+dy<HEIGHT&&!isLand(seed,x+dx,y+dy))return r;
    for(let dy=-r+1;dy<r;dy++)for(const dx of [-r,r])if(x+dx>=0&&x+dx<WIDTH&&y+dy>=0&&y+dy<HEIGHT&&!isLand(seed,x+dx,y+dy))return r;
  }
  return best;
}
function cell(seed,x,y){
  ...
  const coast=coastDistance(seed,x,y),coastal=coast<=2,maritime=clamp((8-coast)/8,0,1);
  const temp=clamp(29-Math.abs(latitude-28)*.48-elevationM*.006-maritime*1.5,-20,29);
  const pacificMoisture=clamp(1-(x-9)/29,0,1),gulfMoisture=clamp(1-Math.hypot(x-38,y-31)/24,0,1),atlanticMoisture=clamp((x-35)/22,0,1);
  ...
  const groundwater=clamp(.18+precipitationMm/2600*.55+(river?.25:0)-elevationM/6000,0,1),drainage=clamp(.25+elevationM/3200+(coastal?.12:0)-(river?.18:0),.08,1);
  let biome;
...
function localTerrain(seed,center,cols=12,rows=8){
  ...
  for(let gy=0;gy<rows;gy++)for(let gx=0;gx<cols;gx++){
    const px=clamp(center.x+gx-Math.floor(cols/2),0,WIDTH-1),py=clamp(center.y+gy-Math.floor(rows/2),0,HEIGHT-1),c=cell(seed,px,py);
...
function ensurePlanet(state,seed){
  if(!state.planet||state.planet.version!==VERSION){
    const s=(Number.isInteger(seed)?seed:(Number.isInteger(state.rng)?state.rng:1))>>>0,start=startingRegion(s);
    state.planet={version:VERSION,seed:s,width:WIDTH,height:HEIGHT,gravityMps2:GRAVITY_MPS2,axialTiltDeg:AXIAL_TILT_DEG,rotationHours:24,atmosphere:{pressureKPa:101.3,oxygenFraction:.2095},current:{x:start.x,y:start.y},startingRegion:regionSummary(start),discoveredRegions:[{x:start.x,y:start.y,firstSeenDay:state.day||1}],overview:atlasSample(s)};
  }
  if(!state.planet.current)state.planet.current={x:state.planet.startingRegion.x,y:state.planet.startingRegion.y};
  if(!state.planet.overview||!Array.isArray(state.planet.overview.cells))state.planet.overview=atlasSample(state.planet.seed);
