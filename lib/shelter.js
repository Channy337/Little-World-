'use strict';
// Persistent physical shelter jobs. All numbers use the engine's internal clock.
const SPEED=42, TIMBER_REQUIRED=6, BUILD_WORK=3;
...
const eligible=a=>!!a&&!a.dead&&(a.age||0)>=12&&!a.home&&!a.pendingHome;
...
function safe(a,s){return !!a&&!a.dead&&(a.age||0)>=12&&!a.home&&a.hunger<72&&a.thirst<50&&a.energy>22&&a.social>25&&!(s.fires||[]).some(f=>distance(a,f)<38);}
function move(a,p,dt){const d=distance(a,p),step=SPEED*dt;if(d<=step||d<.001){a.x=p.x;a.y=p.y;return true;}a.x+=(p.x-a.x)/d*step;a.y+=(p.y-a.y)/d*step;return false;}
...
function repair(s,log=()=>{}){
  if(!Array.isArray(s.constructionSites))s.constructionSites=[];
  const p=project(s);if(!p)return;
  let owner=alive(s,p.ownerId);
  if(!eligible(owner)){
    owner=s.agents.find(eligible);
    if(owner){p.ownerId=owner.id;log(owner.name+' becomes the future resident of the unfinished shelter.','build');}
  }
  p.workerIds=(p.workerIds||[]).filter(id=>alive(s,id)?.shelterJob?.siteId===p.id);
  p.blocked=!owner?'Waiting for a future resident.':!s.agents.some(a=>!a.dead&&knows(a))?'Waiting for someone who knows how to shape timber.':null;
}
function spot(s){
  const bounds=s.worldBounds||{w:480,h:304},center=s.well||s.pond||s.agents[0]||{x:bounds.w/2,y:bounds.h/2};
  const obstacles=[s.well,...(s.buildings||[]),...(s.farms||[]),...(s.plantings||[]),...(s.trees||[]),...(s.rocks||[]),...(s.bushes||[]),...(s.fires||[])].filter(Boolean);
  for(let ring=1;ring<=7;ring++)for(let i=0;i<12;i++){
    ...
    if(s.pond&&distance(p,s.pond)<Math.max(s.pond.w||76,s.pond.h||32)/2+24)continue;
    if(obstacles.every(b=>distance(p,b)>28))return p;
  }
  return null;
}
function assign(s,a,log=()=>{}){
  ...
  if(a.pendingHome){
    const home=s.buildings.find(b=>b.id===a.pendingHome&&b.type==='house');
  ...
  if(!p){
    if(!s.agents.some(v=>!v.dead&&knows(v)))return false;
    const owner=s.agents.find(eligible);if(!owner)return false;
  ...
  p.workerIds.push(a.id);a.workReason='Helping build shelter for '+(alive(s,p.ownerId)?.name||'a villager')+'.';return true;
}
function step(s,a,dt,log=()=>{}){
  ...
  repair(s,log);if(p.blocked){clear(a);return false;}
  ...
  if(job.phase==='fetch'||job.phase==='gather'){
    ...
    a.state='working';job.timer+=dt;if(job.timer>=.35){tree.wood-=1;a.inv.wood=(a.inv.wood||0)+1;delete job.treeId;phase(a,'deliver',p);}return true;
  }
  if(job.phase==='deliver'){
    a.state='moving';if(!move(a,p,dt))return true;
    const timber=Math.min(a.inv.timber||0,Math.max(0,TIMBER_REQUIRED-p.timber)),wood=Math.min(a.inv.wood||0,Math.max(0,TIMBER_REQUIRED-p.timber-timber-p.wood));
    a.inv.timber-=timber;a.inv.wood-=wood;p.timber+=timber;p.wood+=wood;
    if(timber||wood)log(a.name+' delivers '+wood+' wood and '+timber+' timber to the shelter site.','build');job.phase='choose';a.state='idle';return true;
  }
  if(job.phase==='shape'){
    if(!knows(a)){job.phase='choose';return true;}
    if(distance(a,p)>.01){a.state='moving';move(a,p,dt);return true;}
    a.state='working';job.timer+=dt;if(job.timer>=shaping.duration){if(canPay(p,shaping.inputs)&&p.timber<TIMBER_REQUIRED){pay(p,shaping.inputs);produce(p,shaping.outputs);}job.phase='choose';job.timer=0;}return true;
  }
  if(job.phase==='build'){
    if(p.timber<TIMBER_REQUIRED){job.phase='choose';return true;}
    if(!knows(a)){clear(a);return false;}
    if(distance(a,p)>.01){a.state='moving';move(a,p,dt);return true;}
    a.state='working';p.work=Math.min(BUILD_WORK,p.work+dt);const stage=p.work<1?'foundation':p.work<2?'frame':'roof';if(p.stage!==stage){p.stage=stage;log('The shelter '+stage+' takes shape.','build');}
    if(p.work>=BUILD_WORK){
      if(!eligible(resident)){p.blocked='Waiting for a future resident.';return true;}
      s.buildings.push({id:p.id,type:'house',x:p.x,y:p.y,ownerId:p.ownerId,materials:{timber:TIMBER_REQUIRED},methods:['shape:wood'],stock:{wood:p.wood,timber:Math.max(0,p.timber-TIMBER_REQUIRED)}});
      s.constructionSites=[];log('The shelter for '+resident.name+' is complete.','build');for(const v of s.agents)if(v.shelterJob?.siteId===p.id)clear(v);resident.pendingHome=p.id;
...
function validate(s){
  ...
  if(s.constructionSites!==undefined){
    ...
    for(const p of s.constructionSites){if(!integer(p.id)||!integer(p.ownerId)||!Number.isFinite(p.x)||!Number.isFinite(p.y)||!integer(p.wood)||p.wood>6||!integer(p.timber)||p.timber>6||!Number.isFinite(p.work)||p.work<0||p.work>BUILD_WORK||!['materials','foundation','frame','roof'].includes(p.stage)||!Array.isArray(p.workerIds)||p.workerIds.length>2||new Set(p.workerIds).size!==p.workerIds.length||!p.workerIds.every(integer)||!(p.blocked===null||typeof p.blocked==='string'))throw Error('world_invalid');}
  }
  for(const a of s.agents||[]){if(a.pendingHome!==undefined&&!integer(a.pendingHome))throw Error('world_invalid');const j=a.shelterJob;if(j&&(!integer(j.siteId)||!['choose','fetch','gather','deliver','shape','build','movein'].includes(j.phase)||!Number.isFinite(j.timer)||j.timer<0||j.treeId!==undefined&&!integer(j.treeId)))throw Error('world_invalid');}
}
