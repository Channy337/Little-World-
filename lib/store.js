...
function namespace(env) {
  // v2 begins the primitive-origin world without deleting the recoverable v1 key.
  if(env.VERCEL_ENV === 'production') return 'little-world:{production}:v2';
  if(env.VERCEL_ENV === 'preview') {
    ...
    return `little-world:{preview-${branch}}:v2`;
  }
  if(env.VERCEL) throw new Error('environment_missing');
  return 'little-world:{development}:v2';
}
