...
function climateBase(season,climate){
  if(!climate||!Number.isFinite(climate.temperatureMeanC))return BASE_TEMPERATURE[season];
  const mean=climate.temperatureMeanC;
  const amplitude=clamp(8+Math.abs(Number(climate.latitude)||35)*.11+(climate.coastal?-2:2),7,17);
  if(season==='summer')return mean+amplitude*.65;
  if(season==='winter')return mean-amplitude*.75;
  if(season==='autumn')return mean-amplitude*.12;
  return mean+amplitude*.1;
}

function generateDay(random,day,previous,climate){
  const season=seasonForDay(day),roll=random();
  const annualPrecip=climate&&Number.isFinite(climate.precipitationMm)?climate.precipitationMm:1050;
  const wetness=clamp(annualPrecip/1100,.45,1.8);
  const stormChance=((season==='spring'||season==='summer')?.12:.07)*clamp(.72+wetness*.28,.75,1.35);
  const rainChance=(season==='winter'?.22:.32)*clamp(.65+wetness*.35,.65,1.55);
  const storm=roll<stormChance;
  const rain=storm||roll<rainChance;
  ...
  const temperatureC=round(climateBase(season,climate)+(random()-.5)*10+anomaly);
  const humidity=round(clamp((rain?78+random()*20:38+random()*34)+(wetness-1)*12,18,100));
  const tropicalCyclonePossible=!!(climate&&climate.coastal&&temperatureC>=24&&(season==='summer'||season==='autumn'));
  const windKph=round(storm?(25+random()*(tropicalCyclonePossible?75:35)):3+random()*18);
  const precipitationMm=round((storm?14+random()*(tropicalCyclonePossible?68:42):rain?2+random()*17:0)*clamp(.7+wetness*.3,.65,1.35));
  ...
  const groundwater=climate&&Number.isFinite(climate.groundwater)?climate.groundwater:.5;
  const pondLevel=round(clamp(priorPond+precipitationMm*.004-evaporation+(groundwater-.5)*.006,.18,1));
  const soilMoisture=round(clamp(priorSoil+precipitationMm*.008-evaporation*1.8+(groundwater-.5)*.004,.08,1));
  ...
  let hazard=null;
  if(tropicalCyclonePossible&&storm&&windKph>=74&&precipitationMm>=35)hazard='hurricane';
  else if(season==='winter'&&storm&&temperatureC<=2)hazard='blizzard';
  ...
  return {day,season,condition:storm?'storm':snowing?'snow':rain?'rain':cloudCover>45?'cloudy':'clear',hazard,temperatureC,feelsLikeC,humidity,windKph,precipitationMm,cloudCover,storm,snowing,snowDepthCm,pondLevel,soilMoisture,regionalBiome:climate&&climate.biome||null};
}

function ensureWeather(state,random){
  const climate=state&&state.planet&&state.planet.localClimate||null;
  if(!state.weather||typeof state.weather!=='object')state.weather=generateDay(random||Math.random,state.day||1,null,climate);
...
function advanceWeather(state,random){
  ...
  state.weather=generateDay(random,state.day,prior,state.planet&&state.planet.localClimate||null);state.ambientTemperature=state.weather.temperatureC;
  return state.weather;
}
...
function sensoryDescription(weather){
  ...
  const sky=weather.hazard==='hurricane'?'heavy rain and very strong wind':weather.hazard==='blizzard'?'driving snow and strong wind':weather.snowing?'falling snow':weather.condition==='storm'?'dark clouds, hard rain, and strong wind':weather.condition==='rain'?'rainfall':weather.condition==='cloudy'?'clouded skies':'clear skies';
...
function travelMultiplier(weather){
  ...
  if(w.hazard==='hurricane')m*=.32;
  else if(w.hazard==='blizzard')m*=.55;
