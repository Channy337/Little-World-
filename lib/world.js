...
function createWorldService({store,now=Date.now,seed=()=>randomInt(0,4294967296),logger=console.info}={}) {
  ...
  async function tick(heartbeat=false,attempt=0) {
    ...
    logger(JSON.stringify({event:heartbeat?'world_heartbeat':'world_tick',revision:next.revision,simulatedMs,skippedMs,population:state.agents.length,day:state.day}));
