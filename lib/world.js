...
// One real day equals thirty Civoria calendar days (one Civoria month).
const CIVORIA_DAYS_PER_REAL_DAY=30;
...
ensureMinds(initialState);ensureWorldBounds(initialState);ensureKnowledgeSystem(initialState);
...
ensureMinds(startState);ensureWorldBounds(startState);ensureKnowledgeSystem(startState);
...
ensureMinds(state);ensureWorldBounds(state);ensureKnowledgeSystem(state);
if(simulatedMs>0){
