...
const {resolvePending,requestThought,MAX_PER_HEARTBEAT}=require('./ai');
...
function createWorldService({store,now=Date.now,seed=()=>randomInt(0,4294967296),logger=console.info,env=process.env,aiRequest=requestThought,aiTimeoutMs=4000}={}) {
  ...
  async function tick(heartbeat=false,attempt=0,aiBudget={remaining:MAX_PER_HEARTBEAT}) {
    ...
    const engine=createEngine(record.state,1,{aiEnabled:env.CIVORIA_AI==='on'});
    const worldSecondsAdvanced=advanceEngine(engine,simulatedMs);
    const state=engine.snapshot();
    if(heartbeat) await resolvePending(state,{env,request:aiRequest,timeoutMs:aiTimeoutMs,budget:aiBudget});
    ...
    if(await storage().compareAndSwap(raw,nextRaw)!==1) {
      ...
      if(heartbeat){if(attempt<2)return tick(true,attempt+1,aiBudget);throw new Error('world_busy');}
